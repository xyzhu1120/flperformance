...
static void pbuf(void *buf, WORD n, char *s);

void pbuf(void *buf, WORD n, char *s)
{
    WORD i;
    INT8 *cp = (INT8 *)buf;
...
{
    SHA1_CTX c1;
    INT8 res1[] = "34AA973CD4C4DAA4F61EEB2BDBAD27316534 16F";
    INT8 res2[] = "A9993E3647 6816ABA3E25717850C26C9CD0D89D";
    INT32 init_state[5] = {0x67452301,0xEFCDAB89,0x98BADCFE, \
                            0x10325476,0xC3D2E1F0};
    INT8 digest[20];
    WORD i;
        
    sha1_init(&c1,init_state,0);
    for (i = 0; i < 1000000; i++)
        sha1_update(&c1,(INT8 *)"a",1);
    sha1_final(digest,&c1);
    printf("\nSHA1 Test\n");
    pbuf(digest, 20, "Digest is       ");
    printf("Digest should be: %s\n", res1);

    sha1_init(&c1,init_state,0);
    sha1(digest, &c1, (INT8 *)"abc", 3);
    printf("\nSHA1 Test\n");
    pbuf(digest, 20, "Digest is       ");
    printf("Digest should be: %s\n", res2);
}

/* static void hmac_verify(void)
{
    SHA1_CTX c1,c2;
    INT8 res1[] = "EFFCDF6AE5EB2FA2D27416D5F184DF9C259A7C79";
    INT8 *data1 = (INT8 *)"what do ya want for nothing?";
    INT8 *key1 = (INT8 *)"Jefe";
    INT32 init_state[5] = {0x67452301,0xEFCDAB89,0x98BADCFE, \
                            0x10325476,0xC3D2E1F0};
    INT8 prf_key[64];
    ...
    sha1_init(&c1,init_state,0);
    sha1_init(&c2,init_state,0);

    memset(prf_key,0,sizeof(prf_key));
    memcpy(prf_key,key1,strlen((char *)key1));
    for (i = 0; i < sizeof(prf_key); i++)
        prf_key[i] ^= 0x36;
    sha1_update(&c1,prf_key,sizeof(prf_key));
    sha1_update(&c1,(INT8 *)data1,strlen((char *)data1));
    sha1_final(digest,&c1);
    ...
    for (i = 0; i < sizeof(prf_key); i++)
        prf_key[i] ^= 0x5C;
    sha1_update(&c2,prf_key,sizeof(prf_key));
    sha1_update(&c2,(INT8 *)digest,20);
...
static void primitive_verify(void)
{
    INT8 key[] = {0x01,0x23,0x45,0x67,0x89,0xAB,0xCD,0xEF, \
                  0x01,0x12,0x23,0x34,0x45,0x56,0x67,0x78};
    INT8 pt[]  = {0x02,0x13,0x24,0x35,0x46,0x57,0x68,0x79, \
                   0x8A,0x9B,0xAC,0xBD,0xCE,0xDF,0xE0,0xF1};
    INT8 res[] = "524E192F4715C6231F51F6367EA43F18";
    INT32 k1[RC6_TABLE_WORDS];
...
static void umac_verify(void)
{
    ...
    uc = umac_new(key);
    umac(uc, data, data_bytes, tag, nonce);
    umac_delete(uc);
    #if UMAC_MMX_15
    printf("\nUMAC-MMX-15 Test\n");
    pbuf(tag, 16, "Tag is       ");
    printf("Tag should be: %s\n", "8326 1624D5ADA53246188BEFECD9729");
    #elif UMAC_MMX_30
    printf("\nUMAC-MMX-30 Test\n");
    pbuf(tag, 16, "Tag is       ");
    printf("Tag should be: %s\n", "66 9D824D274 44866ED29B5C0AD83C2");
    #elif UMAC_MMX_60
    printf("\nUMAC-MMX-60 Test\n");
    pbuf(tag, 16, "Tag is       ");
    printf("Tag should be: %s\n", "25446229E066311D25EE83B03B12E84D");
    #elif UMAC_STD_30
    printf("\nUMAC-STD-30 Test\n");
    pbuf(tag, 20, "Tag is       ");
    printf("Tag should be: %s\n", "4444E18930295FF528E5 5F95852435DDF5CC85F");
    #elif UMAC_STD_60
    printf("\nUMAC-STD-60 Test\n");
    pbuf(tag, 20, "Tag is       ");
    printf("Tag should be: %s\n", "1FB12F159A9B16CC E1239 EE44016F9EE3A234A");
    #endif
}
...
static void speed_test(void)
{
    ...
    long tag_iters = (1000 * 1024 * 1024) / (data_len * iters_per_tag),
         i, j;
    ...
    printf("MAC'd 1000 MB in %G seconds (%G Gbit/sec).\n", secs, gbits/secs);
}
