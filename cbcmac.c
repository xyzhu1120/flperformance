...
/*
 * AES key schedule (encryption) using new AES instruction
 */
int aesni_setkey_enc( aes_context *ctx, unsigned char *key, int keysize )
{
	switch( keysize )
	{
	...
	printf("addr is %x!\n", (unsigned int)(ctx->buf));
	ctx->rk = (((unsigned int)(ctx->buf))&(~0xf))+16;
	printf("new addr is %x!\n", ctx->rk);
	printf("start init aes key!\n");
	AES_128_Key_Expansion (key, (unsigned char *)(ctx->rk));
	printf("finish init aes key!\n");
	return( 0 );
}
...
//	else length /=16; 
	...
	_mm_storeu_si128 ((__m128i*)out,feedback); 
}
...
/*
 * AES-CBC buffer encryption/decryption using new AES instruction
 */
void aesni_crypt_cbc( aes_context *ctx,
		int mode,
		int length,
		unsigned char iv[16],
		unsigned char *input,
...
//#if defined(POLARSSL_PADLOCK_C) && defined(POLARSSL_HAVE_X86)
	...
	if( mode == AES_ENCRYPT )
	{
		AES_CBC_encrypt(input, output, iv, length, (unsigned char *)(ctx->rk), ctx->nr);
	}
	else if ( mode == AES_DECRYPT )
	{
		AES_CBC_decrypt(input, output, iv, length, (unsigned char *)(ctx->rk), ctx->nr);
	}
	else
	{
		AES_CBC_decrypt_parallelize_4_blocks(input, output, iv, length, (unsigned char *)(ctx->rk), ctx->nr);
	}
}
...
//#if defined(POLARSSL_PADLOCK_C) && defined(POLARSSL_HAVE_X86)
	...
	else
	{
		...
		while( length > 0 )
		{
			...
			aes_crypt_ecb( ctx, mode, output, iv);
...
void cbcmac(unsigned char * input, int len, unsigned char * output, aes_context * ctx)
{
	unsigned char iv[16]="";
	aes_crypt_cbc(ctx, AES_ENCRYPT, len, iv, input, output); 
}

void ncbcmac(unsigned char * input, int len, unsigned char * output, aes_context * ctx)
{
	unsigned char iv[16]="";
	aesni_crypt_cbc(ctx, AES_ENCRYPT, len, iv, input, output); 	
}
